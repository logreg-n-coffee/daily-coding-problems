...
const trappedRainwater = (heights) => {
    let total = 0;
    let left = 0;  // left index - pointer 
    let right = heights.length - 1;  // right index - pointer 
    let leftMax = 0;  // left max 
    let rightMax = 0;  // right max

    while (left <= right) {
        if (leftMax < rightMax) {
            // add the difference between current value
...
const test = [3, 0, 1, 3, 0, 5];
console.log(test);
console.log(trappedRainwater(test));
